...
export const lockers = pgTable('lockers', {
  ...
  estado:        text('estado').notNull().default('libre'), // libre | ocupado | vencido | mantenimiento
...
export const alquileres = pgTable('alquileres', {
  ...
  tipoAlquiler:     text('tipo_alquiler').notNull(), // dia | semana | mes
  montoPagado:      decimal('monto_pagado', { precision: 10, scale: 2 }),
  metodoPago:       text('metodo_pago').notNull().default('efectivo'),
  estado:           text('estado').notNull().default('activo'), // activo | vencido | finalizado
  fechaCreacion:    timestamp('fecha_creacion').defaultNow(),
}, (t) => [
